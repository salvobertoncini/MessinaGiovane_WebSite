...
$username=mysql_real_escape_string($_POST["username_users"]);
$password=mysql_real_escape_string($_POST["password_users"]);
$query = mysql_query("SELECT username_users,password_users FROM users WHERE username_users='".$username."' AND password_users ='".$password."'")  //per selezionare nel db l'utente e pw che abbiamo appena scritto nel log
or DIE('query non riuscita'.mysql_error());
if(mysql_num_rows($query)>0)
{
	$_SESSION["username_users"]=$username; // con questo associo il parametro username che mi è stato passato dal form alla variabile SESSION username
	$_SESSION["password_users"]=$password; // con questo associo il parametro username che mi è stato passato dal form alla variabile SESSION password
	header("location:../gestione/manage.php");
}
else
{
